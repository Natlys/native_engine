...
namespace NC
{
	app_wnd_core_t::app_wnd_core_t() : app_wnd_t() { }
	app_wnd_core_t::~app_wnd_core_t() { }
	// setters //
	// commands //
	v1nil_t app_wnd_core_t::update()
	{
		app_wnd_t::update();
	}
	v1bit_t app_wnd_core_t::remake()
	{
		...
		m_class.lpfnWndProc = app_wnd_core_t::event_proc_init;
		m_class.style = CS_OWNDC | CS_VREDRAW | CS_HREDRAW;
		NC_CHECK(::RegisterClassEx(&m_class), "failed class register", return NC_FALSE);
		...
		raw_dvc.dwFlags = NC_NULL;  // additional param;
		raw_dvc.hwndTarget = NC_NULL; // we don't really need a target;
		NC_CHECK(::RegisterRawInputDevices(&raw_dvc, 1u, sizeof(raw_dvc)), "failed raw register!", return NC_FALSE);
		
		return NC_TRUTH;
	}
	// --==</core_methods>==--
	// --==<impl_methods>==--
	LRESULT WINAPI app_wnd_core_t::event_proc_init(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam) {
		...
		::SetWindowLongPtr(hwnd, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(app_wnd_core_t::event_proc_static));

		return app_wnd_core_t::event_proc_static(hwnd, msg, wparam, lparam);
	}
	LRESULT WINAPI app_wnd_core_t::event_proc_static(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam) {
		if (gui_wapi_event_proc(hwnd, msg, wparam, lparam) == TRUE) { return 0l; }
		return reinterpret_cast<app_wnd_core_t*>(::GetWindowLongPtr(hwnd, GWLP_USERDATA))->event_proc(hwnd, msg, wparam, lparam);
	}
	inline LRESULT WINAPI app_wnd_core_t::event_proc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam) {
		switch (msg) {
		...
		case WM_DROPFILES: {	// wparam is HDROP; lparam is not used;
			...
			NC_CHECK(drop_result, "invalid result!", return NC_NULL);
			...
			m_event_proc(nc_event_applic_t(NC_EVTYPE_APPLIC_DROPF, str_buf));
			return 0l;
			break;
		}
		}
		...
		case WM_INPUT: {
			...
			if (raw_in.header.dwType == RIM_TYPEMOUSE) {
				if (raw_in.data.mouse.lLastX != NC_NULL || raw_in.data.mouse.lLastY != NC_NULL) {
					m_event_proc(nc_cursor_t::nc_event_t(NC_EVTYPE_CURSOR_MOVED, raw_in.data.mouse.lLastX, raw_in.data.mouse.lLastY));
				}
			}
			break;
		}
		...
		case WM_MOUSEMOVE: {
			...
			m_event_proc(nc_cursor_t::nc_event_t(NC_EVTYPE_CURSOR_COORD, coord.x, coord.y));
			return 0l;
			break;
		}
		case WM_MOUSEHWHEEL: {
			m_event_proc(nc_cursor_t::nc_event_t(NC_EVTYPE_CURSOR_SCROL, GET_WHEEL_DELTA_WPARAM(wparam) / WHEEL_DELTA, NC_NULL));
			return 0l;
			break;
		}
		case WM_MOUSEWHEEL: {
			m_event_proc(nc_cursor_t::nc_event_t(NC_EVTYPE_CURSOR_SCROL, NC_NULL, GET_WHEEL_DELTA_WPARAM(wparam) / WHEEL_DELTA));
			return 0l;
			break;
		}
		...
		{
			nc_cursor_t::code_t msb_code = NC_NULL;
			if (msg == WM_LBUTTONDOWN || msg == WM_LBUTTONDBLCLK) { msb_code = NC_CURCODE_LEFT; }
			if (msg == WM_RBUTTONDOWN || msg == WM_RBUTTONDBLCLK) { msb_code = NC_CURCODE_RIGT; }
			if (msg == WM_MBUTTONDOWN || msg == WM_MBUTTONDBLCLK) { msb_code = NC_CURCODE_MIDL; }
			if (::GetCapture() == NC_NULL) { ::SetCapture(m_handle); }
			m_event_proc(nc_cursor_t::nc_event_t(NC_EVTYPE_CURSOR_PRESS, msb_code));
			return 0l;
			break;
		}
		}
		...
		case WM_LBUTTONUP: case WM_RBUTTONUP: case WM_MBUTTONUP: {
			nc_cursor_t::code_t msb_code = NC_NULL;
			if (msg == WM_LBUTTONUP) { msb_code = NC_CURCODE_LEFT; }
			if (msg == WM_RBUTTONUP) { msb_code = NC_CURCODE_RIGT; }
			if (msg == WM_MBUTTONUP) { msb_code = NC_CURCODE_MIDL; }
			m_event_proc(nc_cursor_t::nc_event_t(NC_EVTYPE_CURSOR_RAISE, msb_code));
		...
		case WM_KEYDOWN: case WM_SYSKEYDOWN: {
			m_event_proc(nc_keybod_t::nc_event_t(NC_EVTYPE_KEYBOD_PRESS, wparam));
			return 0l;
			break;
		}
		case WM_KEYUP: case WM_SYSKEYUP: {
			m_event_proc(nc_keybod_t::nc_event_t(NC_EVTYPE_KEYBOD_RAISE, wparam));
			return 0l;
			break;
		}
		case WM_CHAR: case WM_SYSCHAR: {
			m_event_proc(nc_keybod_t::nc_event_t(NC_EVTYPE_KEYBOD_CHART, wparam));
			return 0l;
			break;
		}
		}
		...
		case WM_SIZE: {
			...
			m_event_proc(nc_event_t(NC_EVTYPE_WINDOW_SIZED, get_size_x(), get_size_y()));
			return 0l;
			break;
		}
		case WM_MOVE: {
			set_coord_xy({ LOWORD(lparam), HIWORD(lparam) });
			m_event_proc(nc_event_t(NC_EVTYPE_WINDOW_MOVED, get_coord_x(), get_coord_y()));
			return 0l;
			break;
		}
		case WM_SETFOCUS: {		// wparam is the last window was focused, lParam is not used
			set_focused(NC_TRUTH);
			m_event_proc(nc_event_t(NC_EVTYPE_WINDOW_FOCUS, NC_TRUTH));
			return 0l;
			break;
		}
		case WM_KILLFOCUS: {	// wparam is the next window will be focused, lParam is not used
			set_focused(NC_FALSE);
			m_event_proc(nc_event_t(NC_EVTYPE_WINDOW_FOCUS, NC_FALSE));
			return 0l;
			break;
		}
		case WM_CLOSE: {
			m_event_proc(nc_event_t(NC_EVTYPE_WINDOW_CLOSE, NC_TRUTH));
			::PostQuitMessage(NC_NULL);
			return 0l;
			break;
		}
		case WM_ACTIVATE: {
			m_event_proc(nc_event_t(NC_EVTYPE_WINDOW_ACTIV, NC_TRUTH));
			return 0l;
			break;
		}
		}
