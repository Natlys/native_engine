...
namespace NW
{
	...
	{
		NW_CHECK(app_wnd_t::remake(), "remake error!", return NW_FALSE);
	...
	LRESULT WINAPI app_wnd_core_t::event_proc_init(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam) {
		...
		app_wnd_t* nw_app_window = reinterpret_cast<app_wnd_t*>(crtst->lpCreateParams);
	...
	inline LRESULT WINAPI app_wnd_core_t::event_proc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam) {
		...
		case WM_DROPFILES: {	// wparam is HDROP; lparam is not used;
			...
			m_event_proc(nw_event_applic_t(NW_EVTYPE_APPLIC_DROPF, str_buf));
			return 0l;
			break;
		}
		}
		...
		case WM_INPUT: {
			...
			if (raw_in.header.dwType == RIM_TYPEMOUSE) {
				if (raw_in.data.mouse.lLastX != NW_NULL || raw_in.data.mouse.lLastY != NW_NULL) {
					m_event_proc(nw_cursor_t::nw_event_t(NW_EVTYPE_CURSOR_MOVED, raw_in.data.mouse.lLastX, raw_in.data.mouse.lLastY));
				}
			}
			break;
		}
		...
		case WM_MOUSEMOVE: {
			...
			m_event_proc(nw_cursor_t::nw_event_t(NW_EVTYPE_CURSOR_COORD, coord.x, coord.y));
			return 0l;
			break;
		}
		case WM_MOUSEHWHEEL: {
			m_event_proc(nw_cursor_t::nw_event_t(NW_EVTYPE_CURSOR_SCROL, GET_WHEEL_DELTA_WPARAM(wparam) / WHEEL_DELTA, NW_NULL));
			return 0l;
			break;
		}
		case WM_MOUSEWHEEL: {
			m_event_proc(nw_cursor_t::nw_event_t(NW_EVTYPE_CURSOR_SCROL, NW_NULL, GET_WHEEL_DELTA_WPARAM(wparam) / WHEEL_DELTA));
			return 0l;
			break;
		}
		...
		{
			nw_cursor_t::code_t msb_code = NW_NULL;
			...
			m_event_proc(nw_cursor_t::nw_event_t(NW_EVTYPE_CURSOR_PRESS, msb_code));
			return 0l;
			break;
		}
		}
		...
		case WM_LBUTTONUP: case WM_RBUTTONUP: case WM_MBUTTONUP: {
			...
			m_event_proc(nw_cursor_t::nw_event_t(NW_EVTYPE_CURSOR_RAISE, msb_code));
		...
		case WM_KEYDOWN: case WM_SYSKEYDOWN: {
			m_event_proc(nw_keybod_t::nw_event_t(NW_EVTYPE_KEYBOD_PRESS, wparam));
			return 0l;
			break;
		}
		case WM_KEYUP: case WM_SYSKEYUP: {
			m_event_proc(nw_keybod_t::nw_event_t(NW_EVTYPE_KEYBOD_RAISE, wparam));
			return 0l;
			break;
		}
		case WM_CHAR: case WM_SYSCHAR: {
			m_event_proc(nw_keybod_t::nw_event_t(NW_EVTYPE_KEYBOD_CHART, wparam));
			return 0l;
			break;
		}
		}
		...
		case WM_SIZE: {
			...
			m_event_proc(nw_event_t(NW_EVTYPE_WINDOW_SIZED, get_size_x(), get_size_y()));
			return 0l;
			break;
		}
		case WM_MOVE: {
			set_coord_xy({ LOWORD(lparam), HIWORD(lparam) });
			m_event_proc(nw_event_t(NW_EVTYPE_WINDOW_MOVED, get_coord_x(), get_coord_y()));
			return 0l;
			break;
		}
		case WM_SETFOCUS: {		// wparam is the last window was focused, lParam is not used
			set_focused(NW_TRUE);
			m_event_proc(nw_event_t(NW_EVTYPE_WINDOW_FOCUS, NW_TRUE));
			return 0l;
			break;
		}
		case WM_KILLFOCUS: {	// wparam is the next window will be focused, lParam is not used
			set_focused(NW_FALSE);
			m_event_proc(nw_event_t(NW_EVTYPE_WINDOW_FOCUS, NW_FALSE));
			return 0l;
			break;
		}
		case WM_CLOSE: {
			m_event_proc(nw_event_t(NW_EVTYPE_WINDOW_CLOSE, NW_TRUE));
			::PostQuitMessage(NW_NULL);
			return 0l;
			break;
		}
		case WM_ACTIVATE: {
			m_event_proc(nw_event_t(NW_EVTYPE_WINDOW_ACTIV, NW_TRUE));
			return 0l;
			break;
		}
		}
